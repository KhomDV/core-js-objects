...
function mergeObjects(objects) {
  return objects.reduce((obj, e) => {
    const result = { ...obj };
    Object.entries(e).forEach((data) => {
      if (!result[data[0]]) {
...
function makeWord(lettersObject) {
  const arr = Object.entries(lettersObject).reduce((acc, [key, value]) => {
    value.forEach((i) => {
      acc[i] = key;
    });
    return acc;
...
function sellTickets(queue) {
  ...
  const bill = queue[queue.length - 1];
  const sum = queue.slice(0, -1).reduce((acc, e) => acc + e, 0);
  if ((queue[0] === 25 && sum === bill) || sum === bill - 25) {
    return true;
  }
  return false;
}
